...
public class Player
{
    ...
    public Dictionary<string, int> Gems { get; private set; }
    ...
    public void AddGems(string gemType, int quantity)
    {
        if (!Gems.ContainsKey(gemType))
        {
            Gems[gemType] = 0;
        }
        Gems[gemType] += quantity;
    }

    public bool HasEnoughGems(Dictionary<string, int> cost)
    {
        foreach (var gem in cost)
        {
            if (!Gems.ContainsKey(gem.Key) || Gems[gem.Key] < gem.Value)
            {
                return false;
            }
        }
